...
class Text2SQLTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            dataset_id, llm_model, content, dialect, top_k, retrieval_model = params_result
            ...
            # 如果没有流式响应，尝试获取完整响应
            if not has_streamed_content and hasattr(response, "message") and response.message:
    ...
    def _validate_and_extract_parameters(self, tool_parameters: dict[str, Any]) -> Union[Tuple[str, Any, str, str, int, str], str]:
        ...
        if dialect not in ["mysql", "postgresql", "sqlite", "oracle", "sqlserver"]:
        ...
        if retrieval_model not in ["semantic_search", "keyword_search", "hybrid_search"]:
            return f"不支持的检索模型: {retrieval_model}"
        
        return dataset_id.strip(), llm_model, content.strip(), dialect, top_k, retrieval_model
