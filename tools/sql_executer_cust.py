...
class SQLExecuterTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if not database_url:
            yield self.create_text_message("错误: 必须提供database_url")
            return
        ...
        try:
            ...
            if not self._config_validated:
                yield self.create_text_message("错误: database_url格式无效或不完整")
                return
        except Exception as e:
            yield self.create_text_message(f"错误: 解析database_url失败: {str(e)}")
            return
