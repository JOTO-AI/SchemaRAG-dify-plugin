...
现在可以增加一个可选知识库配置接口允许用户添加示例知识库，通过检索其中的示例加入到提示词中，提高生成sql的准确性
多个知识库检索调用dify使用异步并发，减少检索时长

**使用方法：**
- 新建 `example_dataset_id` 参数知识库ID
- 系统将从知识库中检索相关内容并合并结果
- 将检索到的示例信息添加如text2sql的prompt中
